...
function NavItems({ isModalView = false }) {
  return (
    <div className={`items-center justify-between w-full md:flex md:w-auto ${isModalView ? "" : "hidden"}`} id="nav-items">
      <ul className={`flex flex-col p-4 mt-4 font-medium rounded-lg 
          md:flex-row md:p-0  md:space-x-8 md:mt-0 md:border-0 bg-white ${isModalView ? "border-none":"border border-gray-100"}`}>
        {
          isAdminView ? adminNavOptions.map((item) => (
            <li className="cursor-pointer block py-2 pl-3 pr-4 text-gray-900 rounded-lg md:p-0" key={item.id}>
              {
                item.label
              } 
            </li>
    ...
    <nav className="bg-white fixed w-full z-20 top-0 left-0 border-b border-gray-200">
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto px-4 py-4 xl:py-4 xl:px-0">
        ...
        <div className="flex gap-2">
          ...
          {
            isAuthUser ? <button className="button">Logout</button> : <button className="button" onClick={()=>router.push('/login')}>Login</button>
          }
